...
var vTexAttribLoc;
var samplerAttribLoc;
var alphaAttribLoc;
var lightmapSamplerAttribLoc;
var lightMapTextureObj;
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "KeyU": // reset view to default
            part3Flag = false;
            part2Flag = false;
            break;

        case "KeyM": // reset view to default
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                sorted_models.push([whichSet, inputTriangles[whichSet].material.alpha]);
                var numVerts = inputTriangles[whichSet].vertices.length; // num vertices in tri set
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    vtxToAdd = inputTriangles[whichSet].vertices[whichSetVert]; // get vertex to add
                    normToAdd = inputTriangles[whichSet].normals[whichSetVert]; // get normal to add
                    uvs = inputTriangles[whichSet].uvs[whichSetVert];
                    inputTriangles[whichSet].glVertices.push(vtxToAdd[0],vtxToAdd[1],vtxToAdd[2]); // put coords in set coord list
                    inputTriangles[whichSet].glNormals.push(normToAdd[0],normToAdd[1],normToAdd[2]); // put normal in set coord list
                    inputTriangles[whichSet].glTexCoords.push(1-uvs[0], uvs[1]);
                ...
                gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(inputTriangles[whichSet].glTexCoords),gl.STATIC_DRAW); 
                transparencyBuffers[whichSet] = gl.createBuffer();
                gl.bindBuffer(gl.ARRAY_BUFFER,transparencyBuffers[whichSet]); 
                gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(inputTriangles[whichSet].alpha),gl.STATIC_DRAW); 
                ...
                if(!img_name){
                    inputTriangles[whichSet].noneFlag = true;
                }
                else{
                    inputTriangles[whichSet].noneFlag = false;
                    inputTriangles[whichSet].textureObject = initTexture(img_name);
                }
                lightMapTextureObj = initTexture("lightmap.gif");

            } // end for each triangle set 

            sortedArray = sorted_models.sort(function(a, b) {
                return b[1] - a[1];
            });
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        uniform bool part2flag;
        uniform bool part3flag;
        uniform bool noneFlag;
        ...
        void main(void) {
            ...
            vec4 tex_samp = texture2D(sampler, fragtexCoord);
            vec4 lightmap_samp = texture2D(lightmapSampler, fragtexCoord);
            vec4 tmp;

            if(noneFlag)
                gl_FragColor = vec4(colorOut, 1.0);
            else
                gl_FragColor = tex_samp;

            if(part2flag)
                if (noneFlag){
                    gl_FragColor = vec4(colorOut.rgb*lightmap_samp.rgb, alpha);
                }
                else{
                    tmp = tex_samp * lightmap_samp;
                    gl_FragColor = vec4(tmp.rgb, tmp.a*alpha);
                }
            ...
            if(part3flag){
                if (noneFlag){
                    ...
                    gl_FragColor = vec4(tex_samp.rgb * colorOut.rgb, tex_samp.a*alpha);
                }
            }
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vTexAttribLoc = gl.getAttribLocation(shaderProgram, "vertexCoord");
                gl.enableVertexAttribArray(vTexAttribLoc); 
                

                samplerAttribLoc = gl.getUniformLocation(shaderProgram, "sampler"); 
                alphaAttribLoc = gl.getUniformLocation(shaderProgram, "alpha"); 
                lightmapSamplerAttribLoc = gl.getUniformLocation(shaderProgram, "lightmapSampler"); 

                
                // locate vertex uniforms
                mMatrixULoc = gl.getUniformLocation(shaderProgram, "umMatrix"); // ptr to mmat
                pvmMatrixULoc = gl.getUniformLocation(shaderProgram, "upvmMatrix"); // ptr to pvmmat
                noneFlagULoc = gl.getUniformLocation(shaderProgram, "noneFlag");
                ...
                Part2ULoc = gl.getUniformLocation(shaderProgram, "part2flag");
                Part3ULoc = gl.getUniformLocation(shaderProgram, "part3flag");
...
function renderModels() {
    ...
    for (var i=0; i<sortedArray.length; i++){
        currSet = inputTriangles[sortedArray[i][0]];
        whichTriSet = sortedArray[i][0];
        ...
        gl.uniform1i(Part2ULoc, part2Flag);
        gl.uniform1i(Part3ULoc, part3Flag);
        
        // vertex buffer: activate and feed into vertex shader

        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[whichTriSet].textureObject);
        gl.uniform1i(samplerAttribLoc, 0);
        gl.uniform1i(noneFlagULoc, inputTriangles[whichTriSet].noneFlag);
        gl.uniform1i(samplerAttribLoc, 0);
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, lightMapTextureObj);
        gl.uniform1i(lightmapSamplerAttribLoc, 1);
        gl.uniform1f(alphaAttribLoc, inputTriangles[whichTriSet].material.alpha);
...
function initTexture(img_name) {
    ...
    texture_obj.image.onload = function() {
        ...
        if (isPowerOfTwo(texture_obj.image.width) && isPowerOfTwo(texture_obj.image.height)) {
    ...
    texture_obj.image.crossOrigin = "Anonymous";
